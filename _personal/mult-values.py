...
def runstrat(args=None):
    ...
    _, prices = quote_ctx.get_multiple_history_kline(symble_list, start=args.fromdate, end=args.todate)

    # print(prices)
    for price in prices:
        name = price['code'][0]
        price = price.assign(Data=price.time_key.apply(lambda x:str(x)[0:10])).set_index('Data')
        price = price.drop(columns=['code', 'time_key'])
        price = price.rename(index=str, columns={"open": "Open", "high": "High", "low": "Low", "close": "Close", "volume": "Volume"})
...
def parse_args(pargs=None):
    ...
    parser.add_argument('--data0', default='E:/workspace/code/backtrader/datas/nvda-1999-2014.txt',
                        required=False, help='Data0 to read in')

    parser.add_argument('--data1', default='E:/workspace/code/backtrader/datas/yhoo-1996-2014.txt',
                        required=False, help='Data1 to read in')

    parser.add_argument('--data2', default='E:/workspace/code/backtrader/datas/orcl-1995-2014.txt',
                        required=False, help='Data1 to read in')

    # Defaults for dates
    parser.add_argument('--fromdate', required=False, default='2017-01-01',
                        help='Date[time] in YYYY-MM-DD[THH:MM:SS] format')

    parser.add_argument('--todate', required=False, default='2018-06-07',
                        help='Date[time] in YYYY-MM-DD[THH:MM:SS] format')
